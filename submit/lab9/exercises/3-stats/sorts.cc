...
void
insertionSort(int a[], int n, my_class2* ops)
{
  for (int i = 1; i < n; ++i) {
...
void
bubbleSort(int a[], int n, my_class2* ops)
{
  for (int i = 0; i < n - 1; ++i) {
    for (int j = n - 1; j > i; --j) {
...
void
selectionSort(int a[], int n, my_class2* ops)
{
  for (int i = 0; i < n - 1; ++i) {
    int minIndex = i;
    for (int j = n - 1; j > i; --j) {
...
static void
msort(int a[], int temp[], int lo, int hi, my_class2* ops) {
...
void
mergeSort(int a[], int n, my_class2* ops) 
{
  int* temp = new int[n];
...
/** sort a [lo, hi] */
static void
qsort(int a[], int lo, int hi, my_class2* ops)
{
  if (hi - lo < 1) return;
...
/** sort a [0, n) */
void
quickSort(int a[], int n, my_class2* ops)
{
  qsort(a, 0, n - 1, ops);
}
